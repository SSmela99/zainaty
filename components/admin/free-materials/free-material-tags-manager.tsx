...
import {
  createFreeMaterialTag,
  deleteFreeMaterialTag,
  listFreeMaterialTags,
  updateFreeMaterialTag,
...
import type { FreeMaterialTag } from "@/lib/free-materials/types";
import {
  freeMaterialTagSchema,
  type FreeMaterialTagFormValues,
} from "@/lib/validation/admin-free-materials.schemas";

export function FreeMaterialTagsManager() {
  const [tags, setTags] = useState<FreeMaterialTag[]>([]);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [tagToDelete, setTagToDelete] = useState<FreeMaterialTag | null>(null);
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<FreeMaterialTagFormValues>({
    resolver: yupResolver(freeMaterialTagSchema),
    defaultValues: { name: "" },
  });

  const loadTags = useCallback(async () => {
    setIsLoading(true);
    const result = await listFreeMaterialTags();
    if (result.ok) {
      setTags(result.data);
    } else {
  ...
  function startEdit(tag: FreeMaterialTag) {
    setEditingId(tag.id);
    reset({ name: tag.name });
    setError(null);
  ...
  const onSubmit = handleSubmit((values) => {
    ...
    startTransition(async () => {
      const result = editingId
        ? await updateFreeMaterialTag(editingId, values)
        : await createFreeMaterialTag(values);
  ...
  function requestDelete(tag: FreeMaterialTag) {
    setTagToDelete(tag);
  }
  ...
  function confirmDelete() {
    ...
    startTransition(async () => {
      const result = await deleteFreeMaterialTag(id);
      if (!result.ok) {
    ...
    <div className="space-y-6">
      <AdminPanelCard>
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-5" noValidate>
          <AdminFormField
            label="Nazwa"
            htmlFor="free-material-tag-name"
            error={errors.name?.message}
            required
          >
            <Input
              id="free-material-tag-name"
              placeholder="np. PDF, Checklist, Wideo"
      ...
      <AdminConfirmDialog
        ...
        description={
          tagToDelete
            ? `Czy na pewno chcesz usunąć tag „${tagToDelete.name}"? Materiały z tym tagiem zostaną bez tagu.`
            : ""
        }

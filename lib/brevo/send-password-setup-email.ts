import { escapeHtml } from "./escape-html";
import { emailLogoImgHtml } from "./email-logo";
import { sendEmail } from "./send-email";

function buildPasswordSetupEmailHtml(setupUrl: string): string {
  const safeUrl = escapeHtml(setupUrl);

  return `<!DOCTYPE html>
<html lang="pl">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Twój link do ustawienia hasła</title>
</head>
<body style="margin:0;padding:0;background-color:#142008;font-family:Arial,Helvetica,sans-serif;">
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;">
    Kliknij przycisk, aby ustawić hasło do swojego konta.
  </div>
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;background-color:#142008;">
    <tr>
      <td align="center" style="padding:48px 16px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:480px;background-color:#151414;border-radius:20px;overflow:hidden;">
          <tr>
            <td align="center" bgcolor="#daff02" style="background-color:#daff02;padding:18px 24px;">
              ${emailLogoImgHtml()}
            </td>
          </tr>
          <tr>
            <td style="padding:36px 32px 8px;text-align:center;color:#ffffff;">
              <h1 style="margin:0 0 12px;font-size:24px;line-height:1.25;font-weight:800;color:#ffffff;">
                Twój link do ustawienia hasła
              </h1>
              <p style="margin:0 0 28px;font-size:15px;line-height:1.55;color:#ffffff;">
                Kliknij przycisk, aby ustawić hasło do swojego konta.
              </p>
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin:0 auto 28px;">
                <tr>
                  <td align="center" bgcolor="#daff02" style="border-radius:10px;background-color:#daff02;">
                    <a href="${safeUrl}" target="_blank" style="display:inline-block;padding:14px 28px;font-size:15px;font-weight:800;line-height:1;color:#151414;text-decoration:none;border-radius:10px;">
                      Ustaw hasło
                    </a>
                  </td>
                </tr>
              </table>
              <p style="margin:0 0 10px;font-size:13px;line-height:1.5;color:#d4d4d8;">
                Jeśli przycisk nie działa, skopiuj i wklej ten link w przeglądarce:
              </p>
              <p style="margin:0 0 24px;font-size:12px;line-height:1.5;word-break:break-all;">
                <a href="${safeUrl}" target="_blank" style="color:#b57ae0;text-decoration:underline;">${safeUrl}</a>
              </p>
              <p style="margin:0 0 28px;font-size:13px;line-height:1.5;color:#a1a1aa;">
                Jeśli to nie Ty prosiłeś/aś o link, zignoruj tę wiadomość.
              </p>
            </td>
          </tr>
          <tr>
            <td style="padding:0 32px 28px;text-align:center;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td style="border-top:1px solid #2a2a2a;padding-top:18px;font-size:12px;line-height:1.4;color:#a1a1aa;text-align:center;">
                    © Z AI na Ty -
                    <a href="https://zainaty.pl" style="color:#daff02;text-decoration:none;">zainaty.pl</a>
                  </td>
                </tr>
              </table>
            </td>
          </tr>
        </table>
      </td>
    </tr>
  </table>
</body>
</html>`;
}

export async function sendPasswordSetupEmail(input: {
  email: string;
  setupUrl: string;
}): Promise<void> {
  const text = [
    "Twój link do ustawienia hasła",
    "",
    "Kliknij link, aby ustawić hasło do swojego konta:",
    input.setupUrl,
    "",
    "Jeśli to nie Ty prosiłeś/aś o link, zignoruj tę wiadomość.",
  ].join("\n");

  await sendEmail({
    to: [{ email: input.email }],
    subject: "Twój link do ustawienia hasła",
    text,
    html: buildPasswordSetupEmailHtml(input.setupUrl),
  });
}
